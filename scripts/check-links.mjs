...
//
// This exists mainly to police the vendored `skills/` tree: vendoring copies
// files verbatim, but it does not re-check that their relative links still
// resolve once they are sitting one level down from where elaichi-skills
// keeps them (repo root vs. plugin root). A silently dead link there removes
// a skill's References entry rather than erroring.
